...
float gpuMutualInformation2X1D(
    ...
    auto kernel = mutualInformation2X1DKernels()[
            (n / 1024) + 1
    ];
...
float gpuMutualInformation2X2D(
    ...
    auto kernel = mutualInformation2X2DKernels()[
            (n / 1024) + 1
    ];
    auto gridSize = (n < 65000) ? n : 65000;
    auto blockSize = 1024;
...
float gpuGeneralizedCorrelation2X1D(
    ...
    return (float) std::sqrt(
            1.0 - (float) exp(-2.0 * mutualInformation)
    );
}
...
float gpuGeneralizedCorrelation2X2D(
    ...
    return (float) std::sqrt(
            1.0 - (float) exp(-1.0 * mutualInformation)
    );
}
