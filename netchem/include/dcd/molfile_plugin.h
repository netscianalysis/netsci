...
typedef struct {
  ...
  char altloc[2];     /**< optional PdbLine alternate location code  */
  char insertion[2];  /**< optional PdbLine insertion code           */
...
typedef struct {
   ...
   * case, these would be <_size,0,0> <0,_size,0> and <0,0,_size) for
   ...
   * physical _size of the box, this is the number of voxels in each
...
typedef struct {
  ...
  int wavef_size;               /**< _size of the wavefunction
                                 *   i.e. _size of secular eq. or
...
typedef struct molfile_qm_timestep_metadata {
  ...
  int wavef_size;                      /**< _size of one wavefunction
...
typedef struct {
  ...
  float size;           /* A general _size parameter     */
...
type        data                                     style       _size
----        ----                                     -----       ----
point       x, y, z                                              pixel _size
...
text        x, y, z, up to 24 bytes of text                      pixel _size
color       r, g, b
*/
...
typedef struct {
   ...
   * timestep (like a multi-entry PdbLine), it will have to cache the information
   ...
   * an integer array of the _size nbonds that contains the bond type
   ...
   * Since it is common for users to want to loadCenterOfMassNpyFiles only the final timestep
