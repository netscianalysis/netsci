...
namespace cnpy {
    ...
    template<typename T> void npy_save(std::string fname, const T* data, const std::vector<size_t> shape, std::string mode = "w") {
        ...
        if(fp) {
            //file exists. we need to append to it. read the header, modify the array _size
            size_t word_size;
            bool fortran_order;
            ...
            if(word_size != sizeof(T)) {
                std::cout<<"libnpy error: "<<fname<<" has word _size "<<word_size<<" but npy_save appending data sized "<<sizeof(T)<<"\n";
    ...
    template<typename T> void npz_save(std::string zipname, std::string fname, const T* data, const std::vector<size_t>& shape, std::string mode = "w")
    {
        ...
        if(fp) {
            ...
            //first read the footer. this gives us the offset and _size of the global header
        ...
        local_header += (uint32_t) nbytes; //compressed _size
        local_header += (uint32_t) nbytes; //uncompressed _size
    ...
    template<typename T> std::vector<char> create_npy_header(const std::vector<size_t>& shape) {  
        ...
        header += (char) 0x93;
        header += "NUMPY";
        header += (char) 0x01; //major version of toNumpy format
        header += (char) 0x00; //minor version of toNumpy format
